#!/usr/bin/env python3
"""Flux 1 end-to-end steering workflow.
...
from .compute_vectors import collect_style_representations, TEXT_ENCODER_OUT_LAYERS
from steering.elastic_band import ElasticBandConfig
from .elastic_band import ElasticBandFlux1Runner
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Flux 1 end-to-end steering workflow.")
    ...
    parser.add_argument("--guidance_scale", type=float, default=3.5,
                        help="Guidance scale (default: 3.5 for FLUX.1-dev).")
    ...
    parser.add_argument("--model", type=str, default="black-forest-labs/FLUX.1-dev",
                        help="Flux 1 model ID for steering vector computation.")
    ...
    parser.add_argument("--config", type=Path, default=None,
                        help="Path to elastic-band YAML config. Auto-selected from concept type if omitted "
                             "(configs/flux1_local.yaml for Local Edit, configs/flux1_global.yaml otherwise).")
    return parser
...
def main() -> None:
    ...
    if args.config is not None:
        ...
        config_path = Path("configs/flux1_local.yaml" if concept_type == "Local Edit" else "configs/flux1_global.yaml")
        print(f"Auto-selected config: {config_path}")
    
    if config_path.exists():
        elastic_band_config = ElasticBandConfig.from_yaml(config_path)
    else:
        elastic_band_config = ElasticBandConfig()
