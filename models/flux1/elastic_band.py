#!/usr/bin/env python3
"""Elastic-band search runner for Flux 1.

Contains only the Flux 1-specific runner class. The search algorithm lives in
`steering.elastic_band`.

Usage
-----
python -m models.flux1.elastic_band
"""
...
# ---------------------------------------------------------------------------
# Flux 1-specific runner
# ---------------------------------------------------------------------------


class ElasticBandFlux1Runner:
    ...
    def __init__(
        ...
        self.text_encoder_device = next(t5_encoder.parameters()).device
        ...
        # Encode base prompt embeddings using FLUX.1 T5 dual-encoder multi-layer layout
    ...
    def generate_images(
        ...
        for chunk in batched(uncached, self.batch_size):
            ...
            pooled_embeds = self.pooled_prompt_embeds.repeat(len(chunk), 1)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Flux 1 elastic-band search (standalone).")
    ...
    parser.add_argument("--concept", type=str, default=None,
                        help="Concept name for output subdirectory.")
    ...
    parser.add_argument("--config", type=Path, required=True,
                        help="Path to YAML config file for ElasticBandConfig.")
